...
pub(super) fn document_from_input(theme: DocumentTheme, paragraphs: Vec<InputParagraph>) -> Document {
...
pub(super) fn plain(text: &str) -> InputRun {
  run(text, RunStyles::default())
}

pub(super) fn run(text: &str, styles: RunStyles) -> InputRun {
  InputRun {
    text: text.to_string(),
    styles,
  }
