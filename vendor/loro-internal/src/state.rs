...
impl DocState {
    ...
    /// §perf (flowstate vendor patch): batch variant of [`Self::get_relative_position`]
    /// for a text container — resolves the positions of many live ids in a single
    /// pass. `out[i]` is `None` when `ids[i]` is not currently live (caller falls
    /// back to per-id resolution), or when `container` is missing / not a text
    /// container.
    pub fn get_text_relative_positions(
        &mut self,
        container: &ContainerID,
        ids: &[ID],
        use_event_index: bool,
    ) -> Vec<Option<usize>> {
        let idx = self.arena.register_container(container);
        let Some(state) = self.store.get_container_mut(idx) else {
            return vec![None; ids.len()];
        };
        match state {
            State::RichtextState(s) => s.get_text_indices_of_ids(ids, use_event_index),
            _ => vec![None; ids.len()],
        }
    }
