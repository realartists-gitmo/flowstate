...
impl LoroDoc {
    ...
    /// §perf (flowstate vendor patch): batch-resolve the current event-index
    /// positions of many `ids` in `container`'s text state in a single pass over
    /// the text chunks, instead of O(elements) per id via [`Self::query_pos`].
    /// `out[i]` is `None` when `ids[i]` is not currently live (deleted); callers
    /// that need the history-traced resolution `query_pos` performs for deleted
    /// ids should fall back to `query_pos` for just those ids. Non-text / missing
    /// containers yield all-`None`. Each resolved entry equals the `current.pos`
    /// that `query_pos` returns for a live cursor at that id.
    pub fn query_text_id_positions(&self, container: &ContainerID, ids: &[ID]) -> Vec<Option<usize>> {
        if ids.is_empty() || !self.has_container(container) {
            return vec![None; ids.len()];
        }
        let mut state = self.state.lock();
        state.get_text_relative_positions(container, ids, true)
    }
