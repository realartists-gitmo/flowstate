...
impl Document {
    ...
    /// §flowstate A11.2 (vendor patch): construct a READ-ORIENTED `Document`
    /// from an ALREADY-PARSED typed tree, loading styles from an ALREADY-OPEN
    /// package. Skips the redundant `document.xml` re-parse, the
    /// `numbering.xml`/`core.xml` parses (never consulted by the read APIs —
    /// style resolution walks `self.styles` only), and the package
    /// re-inflate. The stored package is an EMPTY placeholder: the read APIs
    /// (`paragraphs`/`tables`/`styles`/`resolve_*`) never touch it, but
    /// `save`/`to_bytes`/image-embedding on such a `Document` would write that
    /// placeholder — callers on this path must not invoke them.
    pub fn from_parsed_parts(source: &OpcPackage, document: CT_Document) -> Result<Self> {
        let doc_part_name = source.main_document_part().ok_or(Error::NoDocumentPart)?;
        let styles = if let Some(rels) = source.get_part_rels(&doc_part_name) {
            if let Some(styles_rel) = rels.get_by_type(rel_types::STYLES) {
                let styles_part =
                    OpcPackage::resolve_rel_target(&doc_part_name, &styles_rel.target);
                if let Some(styles_xml) = source.get_part(&styles_part) {
                    CT_Styles::from_xml(styles_xml)?
                } else {
                    CT_Styles::new_default()
                }
    ...
    /// §flowstate A11.2 (vendor patch): hand the typed tree back out so the
    /// single parse can outlive this `Document` (the caller parsed it).
    pub fn into_typed_document(self) -> CT_Document {
        self.document
    }
