    ...
    static void dot_product_attention(const StorageView& queries,
      ...
      if (relative_attention_bias) {
        ...
        if (position_bias->empty()) {
          const dim_t query_length = queries.dim(2);
          const dim_t key_length = keys.dim(2);
          // §perf: the query block occupies the LAST `query_length` key positions, so its
          // start offset is `key_length - query_length`. This reduces to `key_length - 1` for
          // the normal single-token decode (query_length == 1) but is also correct when
          // verifying multiple drafted tokens against a non-empty cache (prompt-lookup
          // speculative decoding), where query_length > 1.
          *position_bias = compute_relative_bias(*relative_attention_bias,
                                                 query_length,
                                                 key_length,
                                                 maximum_relative_position,
                                                 is_decoder,
                                                 with_cache ? key_length - query_length : 0);
        }
