...
namespace ctranslate2 {
  namespace models {
    ...
    EncoderDecoderReplica::run_translation(const std::vector<std::vector<std::string>>& source,
      ...
      // §perf: prompt-lookup speculative decoding — hand the encoder source ids to the
      // greedy search so it can draft continuations copied from the input (+ recurring
      // output). Verification is lossless; see decoding.cc::greedy_prompt_lookup.
      decoding_options.prompt_lookup_ngram = options.prompt_lookup_ngram;
      decoding_options.prompt_lookup_max_tokens = options.prompt_lookup_max_tokens;
      if (options.prompt_lookup_ngram > 0 && !source_ids.empty())
        decoding_options.prompt_lookup_source = source_ids[0];
