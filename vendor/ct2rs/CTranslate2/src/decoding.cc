...
namespace ctranslate2 {
  ...
  // §perf: trim every self-attention KV-cache tensor in `state` to keep time positions
  // [0, new_length). Cross-attention ("memory*") caches are encoder-derived and constant,
  // so they are left untouched. Used after a partial-accept in prompt-lookup speculation to
  // discard the cache entries produced by rejected drafted tokens, so the next real step is
  // appended at the correct offset (which keeps the position encoding / cache consistent).
  static void truncate_self_attention_state(layers::DecoderState& state, dim_t new_length) {
    for (auto& pair : state) {
      const std::string& name = pair.first;
      StorageView& cache = pair.second;
      if (!starts_with(name, "self_") || !cache)
        continue;
      // rank-4 [batch, heads, time, head_dim] -> time is dim 2;
      // rank-3 [batch, time, heads*head_dim]  -> time is dim 1 (merged multi-query).
      const dim_t time_dim = (cache.rank() == 4) ? 2 : 1;
      if (new_length >= cache.dim(time_dim))
        continue;
      StorageView trimmed(cache.dtype(), cache.device());
      ops::Slide(time_dim, /*index=*/0, /*size=*/new_length)(cache, trimmed);
      cache = std::move(trimmed);
    }
  }

  // §perf: prompt-lookup drafting. The pattern is the last `ngram` tokens of `generated`.
  // Find its most recent earlier occurrence — first in `source` (copy-from-input), then in
  // the generated history (recurring output, e.g. repeated JSON scaffolding) — and return up
  // to `max_tokens` tokens that followed it. Empty if no continuation is found. This is a
  // pure guess; the caller verifies every drafted token against the model, so a wrong guess
  // only costs a rejected batched forward, never correctness.
  static std::vector<size_t> prompt_lookup_draft(const std::vector<size_t>& source,
                                                 const std::vector<size_t>& generated,
                                                 size_t ngram,
                                                 size_t max_tokens) {
    if (ngram == 0 || generated.size() < ngram || max_tokens == 0)
      return {};
    const size_t g = generated.size();
    const auto scan = [&](const std::vector<size_t>& hay) -> std::vector<size_t> {
      if (hay.size() <= ngram)
        return {};
      // scan candidate start positions from most-recent to oldest
      for (size_t start = hay.size() - ngram; ; --start) {
        bool match = true;
        for (size_t j = 0; j < ngram; ++j) {
          if (hay[start + j] != generated[g - ngram + j]) { match = false; break; }
        }
        if (match) {
          std::vector<size_t> draft;
          for (size_t k = start + ngram; k < hay.size() && draft.size() < max_tokens; ++k)
            draft.push_back(hay[k]);
          if (!draft.empty())
            return draft;
        }
        if (start == 0)
    ...
    std::vector<size_t> draft = scan(source);   // prefer copy-from-source
    if (!draft.empty())
      return draft;
    return scan(generated);                      // else recurring output (self-match on the
                                                 // tail yields an empty draft and is skipped)
  }

  GreedySearch::GreedySearch(const float length_penalty,
                             const float coverage_penalty,
                             std::function<bool(DecodingStepResult)> callback,
                             std::vector<std::vector<size_t>> prompt_lookup_source,
                             size_t prompt_lookup_ngram,
                             size_t prompt_lookup_max_tokens)
    : _length_penalty(length_penalty)
    , _coverage_penalty(coverage_penalty)
    , _callback(std::move(callback))
    , _prompt_lookup_source(std::move(prompt_lookup_source))
    , _prompt_lookup_ngram(prompt_lookup_ngram)
    , _prompt_lookup_max_tokens(prompt_lookup_max_tokens)
  {
  }

  // §perf: lossless prompt-lookup speculative greedy decode for a single sequence.
  // Engaged only under the gate in GreedySearch::search (greedy argmax sampler, no logits
  // processors, batch size 1, no prefix, start_step 0, min_length 0, no penalties), which
  // guarantees each committed token is a pure argmax — so verifying a batch of drafted
  // tokens in one forward and accepting the longest argmax-matching run reproduces plain
  // greedy exactly. `source` is the encoder source ids already remapped into the decoder's
  // output id-space, so it shares the id-space of the sampled tokens.
  static std::vector<DecodingResult>
  greedy_prompt_lookup(layers::Decoder& decoder,
                       layers::DecoderState& state,
                       const Sampler& sampler,
    ...
    const Device device = decoder.device();
    const DataType dtype = decoder.output_type();
    std::vector<DecodingResult> results(1);
    results[0].hypotheses.resize(1);
    if (return_scores)
      results[0].scores.resize(1, 0.f);
    std::vector<size_t>& hyp = results[0].hypotheses[0];

    const auto is_end = [&](size_t id) {
      return std::find(end_ids.begin(), end_ids.end(), id) != end_ids.end();
    };

    StorageView logits(dtype, device);
    size_t cur = start_id;      // next token to feed (output id-space)
    dim_t cache_len = 0;        // KV-cache time length == tokens fed so far
    bool finished = false;

    // §perf: env-gated instrumentation. The GPU speedup ceiling is tokens-committed per
    // forward pass (on GPU each forward is ~constant cost regardless of width).
    const bool spec_stats = (std::getenv("FLOWSTATE_SPEC_STATS") != nullptr);
    long num_forwards = 0;
    long drafted_total = 0;
    long accepted_from_draft = 0;

    while (!finished && static_cast<dim_t>(hyp.size()) < max_length) {
      const std::vector<size_t> draft = prompt_lookup_draft(source, hyp, ngram, max_tokens);
      const dim_t k = static_cast<dim_t>(draft.size());
      const dim_t n = 1 + k;

      std::vector<int32_t> feed;
      feed.reserve(n);
      feed.push_back(static_cast<int32_t>(cur));
      for (size_t d : draft)
        feed.push_back(static_cast<int32_t>(d));

      StorageView ids({1, n}, feed, device);
      convert_to_original_word_ids(decoder, ids);
      // Step form with the current cache length as the position base: the n fed tokens occupy
      // positions [cache_len, cache_len+n) with causal self-attention over the existing cache.
      // (The lengths form passes step=-1 -> positions from 0; it is for fresh prompt processing
      // only and produces wrong positions when continuing from a non-empty cache.)
      decoder(cache_len, ids, state, &logits);     // logits: [1, n, vocab]
      cache_len += n;
      ++num_forwards;
      drafted_total += k;

      StorageView row_logits(logits.dtype(), device);
      row_logits.copy_from(logits);
      row_logits.reshape({n, logits.dim(-1)});
      if (return_scores)
        ops::LogSoftMax()(row_logits);
      StorageView best_ids(DataType::INT32);
      StorageView best_probs(dtype);
      sampler(row_logits, best_ids, best_probs);

      // best_ids[p] is the true token following the p-th fed token; it is a valid greedy
      // token only while every earlier drafted token matched its argmax.
      dim_t m = 0;
      while (m < k && static_cast<size_t>(best_ids.at<int32_t>(m)) == draft[m])
        ++m;
      accepted_from_draft += m;

      for (dim_t j = 0; j <= m; ++j) {
        const size_t tok = static_cast<size_t>(best_ids.at<int32_t>(j));
        if (return_scores)
          results[0].scores[0] += best_probs.scalar_at<float>({j, 0});
        if (is_end(tok)) {
          if (include_eos_in_hypotheses)
            hyp.push_back(tok);
          finished = true;
          break;
        }
        hyp.push_back(tok);
        if (static_cast<dim_t>(hyp.size()) >= max_length) {
          finished = true;
          break;
        }
      }
      ...
      // No EOS/length break: best_ids[m] is the last accepted token and has NOT been fed.
      // Positions [0, prev_len + m) hold the prefix + the m matched drafts (all correct);
      // discard the rejected tail so the next step is appended at the right offset.
      cur = static_cast<size_t>(best_ids.at<int32_t>(m));
      const dim_t keep_len = (cache_len - n) + m + 1;
      truncate_self_attention_state(state, keep_len);
      cache_len = keep_len;
    }

    // Match the normal path's score normalization (coverage penalty is gated to 0, so no
    // attention is needed). pow(len, 0) == 1, so this is a no-op when length_penalty == 0.
    if (return_scores && !hyp.empty())
      results[0].scores[0] = finalize_hypothesis_score(results[0].scores[0],
                                                       static_cast<float>(hyp.size()),
                                                       length_penalty, 0.f, nullptr);

    if (spec_stats)
      std::fprintf(stderr,
                   "SPEC committed=%zu forwards=%ld tok_per_fwd=%.2f drafted=%ld accepted=%ld "
                   "accept_rate=%.2f\n",
                   hyp.size(), num_forwards,
                   num_forwards ? double(hyp.size()) / double(num_forwards) : 0.0,
                   drafted_total, accepted_from_draft,
                   drafted_total ? double(accepted_from_draft) / double(drafted_total) : 0.0);
  ...
  GreedySearch::search(layers::Decoder& decoder,
    ...
    // §perf: lossless prompt-lookup speculative fast path. Only when the whole config makes
    // each token a pure argmax (no logits processors => no repetition/no-repeat/suppress,
    // greedy sampler already ensured by make_search_strategy setting _prompt_lookup_ngram),
    // single sequence, no prefix, no penalties, no per-step callback/attention, min_length 0
    // and start_step 0. Output is then identical to the loop below.
    if (_prompt_lookup_ngram > 0
        && !_prompt_lookup_source.empty()
        && start_ids.size() == 1
        && !prefix_ids
        ...
        && !return_logits_vocab
        && !_callback
        && _coverage_penalty == 0    // coverage needs attention, which this path doesn't gather
        && start_step == 0
        && min_length == 0) {        // length_penalty is fine: greedy tokens are unaffected;
                                     // the score is normalized inside greedy_prompt_lookup
      return greedy_prompt_lookup(decoder, state, sampler, start_ids[0], end_ids,
                                  _prompt_lookup_source[0], _prompt_lookup_ngram,
                                  _prompt_lookup_max_tokens, max_length, _length_penalty,
                                  return_scores, include_eos_in_hypotheses);
    }
  ...
  make_search_strategy(const DecodingOptions& options) {
    if (options.beam_size == 1 && options.prefix_bias_beta == 0) {
      // §perf: enable prompt-lookup only for a greedy (argmax) sampler; random sampling
      // would make the batched verify non-lossless.
      const bool greedy_sampler = (options.sampling_topk == 1
                                   && options.sampling_topp >= 1.f
                                   && options.sampling_temperature == 1.f);
      const size_t lookup_ngram = greedy_sampler ? options.prompt_lookup_ngram : 0;
      return std::make_unique<GreedySearch>(options.length_penalty,
                                            options.coverage_penalty,
                                            options.callback,
                                            options.prompt_lookup_source,
                                            lookup_ngram,
                                            options.prompt_lookup_max_tokens);
    } else
  ...
  decode(layers::Decoder& decoder,
    ...
    if (decoder.output_layer_is_updated()) {
      ...
      // §perf: prompt-lookup source must share the id-space of the sampled tokens, which are
      // in output space when the output layer is restricted. Drop any source id absent from
      // the restricted output set (it could never be generated, so it can never be drafted).
      for (auto& seq : options.prompt_lookup_source)
        seq = map_to_output_word_ids(decoder, seq);
    }
