...
namespace ctranslate2 {
  ...
  public:
    ...
    GreedySearch(const float length_penalty = 0,
                 ...
                 size_t prompt_lookup_ngram = 0,
                 size_t prompt_lookup_max_tokens = 10);
  ...
  private:
    ...
    const std::vector<std::vector<size_t>> _prompt_lookup_source;
    const size_t _prompt_lookup_ngram;
    const size_t _prompt_lookup_max_tokens;
  };
  ...
  struct DecodingOptions {
    ...
    // §perf: prompt-lookup speculative decoding (lossless). Draft continuations by
    // matching the tail of the generated sequence against `prompt_lookup_source` (the
    // encoder source ids, already remapped into the decoder id-space) plus the
    // generated-so-far, then verify all drafted tokens in ONE batched decoder forward
    // and accept the longest run whose argmax matches the draft. Output is identical to
    // plain greedy. `prompt_lookup_ngram == 0` disables it. Only engaged for greedy
    // search, batch size 1, with no sequence-dependent logits processing (see
    // make_search_strategy gate) so verification is provably lossless.
    std::vector<std::vector<size_t>> prompt_lookup_source;
    size_t prompt_lookup_ngram = 0;
    size_t prompt_lookup_max_tokens = 10;
  };
