...
namespace ctranslate2 {
  ...
  struct TranslationOptions {
    ...
    // §perf: prompt-lookup speculative decoding (lossless). When > 0, greedy decoding drafts
    // continuations by matching the last `prompt_lookup_ngram` generated tokens against the
    // source (and generated-so-far) and verifies up to `prompt_lookup_max_tokens` drafted
    // tokens per step in a single batched forward. Output is identical to plain greedy. Only
    // engaged for greedy argmax, batch size 1, min_decoding_length 0, no penalties/processors.
