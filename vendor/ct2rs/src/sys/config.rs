...
pub struct Config {
    ...
    /// Number of model replicas per device — CTranslate2 `inter_threads`, i.e. how many decodes may
    /// run concurrently. 0/1 = a single replica (decodes serialize). (default: 0)
    ///
    /// FLOWSTATE PATCH: upstream ct2rs does not expose this; the convenience translator constructor
    /// hard-codes a single replica. Honoured by the `translator` factory (see `include/translator.h`).
