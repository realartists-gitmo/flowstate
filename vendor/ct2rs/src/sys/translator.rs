...
pub struct TranslationOptions<T: AsRef<str>, U: AsRef<str>> {
    ...
    /// §perf: prompt-lookup speculative decoding n-gram length (0 disables). When > 0, greedy
    /// decoding drafts continuations copied from the source (and generated-so-far) and verifies
    /// them in one batched forward — output identical to plain greedy. Greedy/batch-1 only.
    /// (default: 0)
    pub prompt_lookup_ngram: usize,
    /// §perf: maximum drafted tokens verified per step under prompt-lookup. (default: 10)
    pub prompt_lookup_max_tokens: usize,
...
impl<T: AsRef<str>, U: AsRef<str>> TranslationOptions<T, U> {
    fn to_ffi(&self) -> ffi::TranslationOptions {
        ffi::TranslationOptions {
            ...
            prompt_lookup_max_tokens: self.prompt_lookup_max_tokens,
