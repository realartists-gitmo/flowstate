...
impl<B: BTreeTrait> BTree<B> {
    ...
    /// §flowstate vendor patch (bimodal-undo fix): the first leaf strictly
    /// AFTER `path.leaf` in document order whose cache satisfies `pred`,
    /// skipping whole subtrees whose aggregated caches fail it —
    /// O(log n · branch factor) where the `next_elem` chain is linear.
    ///
    /// `pred` must be MONOTONE over subtree aggregation: if any descendant
    /// leaf's cache satisfies it, every ancestor cache must too (true for
    /// counting caches such as "number of elements with property P > 0").
    /// Returns `None` when no later leaf matches.
    /// §flowstate debug helper: print each ancestor level's child caches for
    /// `leaf`, marking the child on the path. Diagnostic only.
    pub fn debug_dump_ancestor_caches(&self, leaf: LeafIndex, fmt: &mut dyn FnMut(&B::Cache) -> String) {
        let mut node_idx: ArenaIndex = leaf.into();
        let mut level = 0;
        loop {
            let (parent_idx, slot) = match node_idx {
                ArenaIndex::Leaf(leaf_idx) => {
                    let Some(leaf_node) = self.leaf_nodes.get(leaf_idx) else {
                        eprintln!("    dump: leaf {leaf_idx:?} missing");
                        return;
                    };
                    let parent = self.get_internal(leaf_node.parent());
                    (leaf_node.parent(), Self::get_leaf_slot(leaf_idx, parent))
                }
                ArenaIndex::Internal(_) => {
                    let node = self.get_internal(node_idx);
                    match node.parent {
                        Some(p) => (p, node.parent_slot as usize),
                        None => return,
                    }
                }
            };
            let parent = self.get_internal(parent_idx);
            let caches: Vec<String> = parent
                .children
                .iter()
                .enumerate()
                .map(|(i, c)| {
                    let mark = if i == slot { "*" } else { "" };
                    format!("[{i}{mark} {}]", fmt(&c.cache))
                })
                .collect();
            eprintln!("    dump level {level}: slot {slot}: {}", caches.join(" "));
            node_idx = parent_idx;
            level += 1;
        }
    }

    pub fn next_leaf_matching(
        &self,
        path: Cursor,
        pred: &mut dyn FnMut(&B::Cache) -> bool,
    ) -> Option<LeafIndex> {
        let mut node_idx: ArenaIndex = path.leaf.into();
        loop {
            // Climb one level: identify our slot in the parent, then scan the
            // siblings to our right.
            let (parent_idx, slot) = match node_idx {
                ArenaIndex::Leaf(leaf_idx) => {
                    let leaf = self.leaf_nodes.get(leaf_idx)?;
                    let parent = self.get_internal(leaf.parent());
                    (leaf.parent(), Self::get_leaf_slot(leaf_idx, parent))
                }
                ArenaIndex::Internal(_) => {
                    let node = self.get_internal(node_idx);
                    (node.parent?, node.parent_slot as usize)
                }
            };
            let parent = self.get_internal(parent_idx);
            for child in parent.children.iter().skip(slot + 1) {
                if !pred(&child.cache) {
                    continue;
                }
                // Descend to the leftmost matching leaf of this subtree.
                let mut cur = child.arena;
                loop {
                    match cur {
                        ArenaIndex::Leaf(_) => return Some(cur.unwrap_leaf().into()),
                        ArenaIndex::Internal(_) => {
                            let node = self.get_internal(cur);
                            cur = node.children.iter().find(|c| pred(&c.cache))?.arena;
                        }
                    }
                }
            }
