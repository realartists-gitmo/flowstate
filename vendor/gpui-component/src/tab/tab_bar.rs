...
use gpui::{
    ...
    Hsla, InteractiveElement, IntoElement, ParentElement, Pixels, RenderOnce, ScrollHandle, SharedString,
...
impl TabBar {
    ...
    pub fn active_tab_bg(mut self, color: Hsla) -> Self {
        self.active_tab_bg = Some(color);
        self
    }

    pub fn active_tab_fg(mut self, color: Hsla) -> Self {
        self.active_tab_fg = Some(color);
        self
    }
    ...
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        for (ix, child) in self.children.into_iter().enumerate() {
            ...
            let mut tab = child
                ...
                .with_size(self.size)
                .when_some(self.active_tab_bg, |tab, color| tab.active_bg(color))
                .when_some(self.active_tab_fg, |tab, color| tab.active_fg(color));
