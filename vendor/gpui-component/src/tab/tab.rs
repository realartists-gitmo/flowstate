...
pub struct Tab {
    ...
    context_menu: Option<
        Rc<dyn Fn(PopupMenu, &mut Window, &mut gpui::Context<PopupMenu>) -> PopupMenu + 'static>,
    >,
}
...
impl Tab {
    ...
    pub fn active_fg(mut self, color: Hsla) -> Self {
...
impl RenderOnce for Tab {
    fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        if let Some(active_bg) = self.active_bg {
            selected_style.bg = active_bg.into();
        }
        if let Some(active_fg) = self.active_fg {
            selected_style.fg = active_fg;
        }
