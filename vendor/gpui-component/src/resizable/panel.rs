...
impl ResizablePanel {
    ...
    /// Set whether this panel should absorb free space in the group.
    ///
    /// The default is `true`, preserving the historical behavior where all
    /// panels flex-grow. Set this to `false` for sidebars/toolkits that should
    /// keep their configured size until the user explicitly drags a handle.
