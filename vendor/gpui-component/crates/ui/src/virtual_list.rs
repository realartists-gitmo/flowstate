    ...
    V: Render,
{
    ...
    VirtualList {
        ...
        // FLOWSTATE PATCH: see `VirtualList::with_fixed_cross_axis_size`.
...
impl VirtualList {
    ...
    /// FLOWSTATE PATCH: set the list's non-scrolling axis size when the caller
    /// already knows it.
    ///
    /// For vertical lists this is the row width; for horizontal lists this is
    /// the row height. Supplying it skips [`Self::measure_item`] entirely,
    /// where [`Self::with_item_to_measure_index`] only changes *which* item is
    /// measured. Measuring means laying out a real row (for flowstate, a
    /// paragraph of rich text) on every layout pass purely to learn a width we
    /// already have.
...
impl Element for VirtualList {
    ...
    fn request_layout(
        ...
        // FLOWSTATE PATCH: skip the measure pass when the caller already knows
        // the cross-axis extent. Only the cross-axis component of this value is
        // ever read (see the `content_size` assignments below), so the along-axis
        // component is left at zero.
        let longest_item_size = match self.fixed_cross_axis_size {
            Some(size) if self.axis.is_horizontal() => Size {
                width: px(0.),
                height: size,
            },
            Some(size) => Size {
            ...
            None => self.measure_item(None, window, cx),
        };
