...
fn first_item_ending_after(sizes: &[Pixels], origins: &[Pixels], target: Pixels) -> usize {
    let mut low = 0;
    let mut high = sizes.len().min(origins.len());
    while low < high {
        let mid = low + (high - low) / 2;
        if origins[mid] + sizes[mid] > target {
            high = mid;
        } else {
            low = mid + 1;
        }
...
impl Element for VirtualList {
    ...
    fn request_layout(
        ...
        let longest_item_size = if let Some(cross_axis_size) = self.fixed_cross_axis_size {
            ...
            self.measure_item(list_width, window, cx)
        };
                ...
                size_layout = window.with_element_state(
                    ...
                    |state: Option<ItemSizeLayout>, _window| {
                        ...
                        if !Rc::ptr_eq(&state.items_sizes, &self.item_sizes) {
                            ...
                            for (i, item_size) in self.item_sizes.iter().enumerate() {
                                origins.push(cumulative);
                                let item_size = item_size.along(self.axis)
                                    + if i + 1 == self.items_count { px(0.) } else { gap };
            ...
            |_style, _, hitbox, window, cx| {
                if self.items_count > 0 {
                    ...
                    let first_visible_element_ix =
                        first_item_ending_after(item_sizes, item_origins, start);
                    let last_visible_element_ix = first_item_ending_after(item_sizes, item_origins, end)
                        .saturating_add(2)
