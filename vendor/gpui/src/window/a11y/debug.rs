...
fn node_to_json(
    ...
    // FLOWSTATE PATCH: text-run + selection properties.
    //
    // Upstream's dump omits these, which makes caret and selection position
    // unassertable from the debug JSON — precisely what a headless test of a
    // text editor most needs to check. `character_lengths` is also the cheapest
    // way to verify a run was chunked and its UTF-8 lengths are coherent.
    if !node.character_lengths().is_empty() {
        aria.insert(
            "character_lengths".into(),
            json!(node.character_lengths().to_vec()),
        );
    }
        ...
        let pos = |p: &accesskit::TextPosition| {
            json!({
                "node": ephemeral
                    .get(&p.node)
                    .cloned()
                    .unwrap_or_else(|| p.node.0.to_string()),
                "character_index": p.character_index,
            })
        };
        aria.insert(
            "text_selection".into(),
            json!({ "anchor": pos(&sel.anchor), "focus": pos(&sel.focus) }),
        );
    }
