...
pub(crate) struct TestWindowState {
    ...
    is_fullscreen: bool,
    /// FLOWSTATE PATCH: retained so tests can dispatch AccessKit action requests
    /// the way a real assistive-technology client would. See `a11y_init`.
    pub(crate) a11y: Option<crate::A11yCallbacks>,
}
...
impl PlatformWindow for TestWindow {
    /// FLOWSTATE PATCH: activate accessibility immediately.
    ///
    /// The trait's default is a no-op, so on the test platform the activation
    /// callback never fires, `A11y::is_active()` stays false, and `draw_roots`
    /// skips `begin_frame` entirely — meaning no tree is built and
    /// [`crate::Window::debug_a11y_tree_json`] always returns `None`. That makes
    /// the accessibility tree unassertable from `#[gpui::test]`, which is the
    /// only way to test it without a live screen reader.
    ///
    /// Activating unconditionally is the right default for a test platform: it
    /// is the deterministic counterpart of a real adapter seeing an assistive
    /// technology connect, and it is symmetric with the already-existing
    /// force-OFF switch, [`crate::Application::new_inaccessible`] (tests that
    /// want a11y off can use that). The callbacks are retained so tests can
    /// drive `AccessKit` action requests.
    fn a11y_init(&self, callbacks: crate::A11yCallbacks) {
        (callbacks.activation)();
        self.0.lock().a11y = Some(callbacks);
    }
