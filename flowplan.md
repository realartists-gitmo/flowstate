...
1. Ok, the zoom is still not maintaining fidelity, but I think I have visual data that might help pin the problem down. For the flow, if I zoom out to minimum, then zoom in as fast as I can, I go to the far far right of the flow when zoomed in. But if I zoom in slower, I go left of that---and the slower I go, the more left I end up. At the absolutely slowest I tried, the screen found the rough midpoint. 2. In the middle of the zooming process there's a lot of wobbly snapping back and forth where elements on the screen will tick their position between two seeming states rather than smoothly scaling. 3. For the dot grid, they sort of seem to change position or shift around as zoom occurs, but I can't pin down the rule it follows---but as I zoom in, the dots get fainter and less defined, and as I zoom out, they become pronounced and very defined---logically, the opposite should happen---zooming on the dots should make them bigger. I presume this is because the dots are point-like, and they just represent as points on whatever the given zoom is? Or is it a more complex explanation? Regardless, the dot sizing at 100% zoom is perfect, it would be great if that scaled with the zoom logically.
