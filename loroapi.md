...
# FILE: .next/standalone/pages/docs/api/js.mdx
...
**âš ï¸ Critical Pitfall:** Never let two parallel peers (e.g., multiple tabs/devices) share the same PeerID â€” it creates duplicate op IDs and causes document divergence. Common mistakes:
...
**âš ï¸ Important:** This setting doesn't persist in exported Updates or Snapshots. You must reapply this configuration each time you initialize a document.
...
**ðŸ“ Note:** Creating root containers (e.g., `doc.getText("...")`) does not record operations; nested container creation (e.g., `map.setContainer(...)`) does.

**âš ï¸ Pitfall:** Avoid concurrent creation of regular child containers with the same key in LoroMaps. Instead of:
...
<Indent>
**âš ï¸ Important:** In Loro 1.0, `version()`/`frontiers()` include pending (uncommitted) local operations.

**ðŸ“ Note:** After `checkout()`, the document enters "detached" mode and becomes read-only by default. Use `attach()` or `checkoutToLatest()` to return to editing mode. See [Version Deep Dive](/docs/advanced/version_deep_dive) and [Attached vs Detached States](/docs/concepts/attached_detached).
...
**âš ï¸ Important:** Events are emitted synchronously as of v1.8. If you are pinned to `<=1.7.x`, await a microtask before reading the batch.
...
**ðŸ“ Note:** Multiple operations before a commit are batched into a single event. See [Event Handling](/docs/tutorial/event).
...
**âš ï¸ Critical Distinction:** Loro transactions are NOT ACID database transactions:
...
**ðŸ“ Note:** Frontiers are a compact version representation.

**âš ï¸ Limitation:** When you have a Frontier pointing to operations you don't know about, you cannot determine the complete set of operation IDs included in that version. Version Vectors don't have this limitation but are more verbose. See [Frontiers](/docs/concepts/frontiers) for trade-offs.
...
These helpers relate to shallow snapshots and redaction. If you need a refresher on what â€œshallowâ€ means, see [Shallow Snapshots](/docs/concepts/shallow_snapshots).
...
**âš ï¸ Critical: UTF-16 String Encoding**
...
console.log(text.toString()[6]); // âš ï¸ Invalid - splits the emoji
text.delete(6, 2); // âœ… Correct - deletes entire emoji
text.delete(6, 1); // âŒ Wrong - corrupts the emoji
...
const end = enc.encode("Hi ðŸ‘‹").length; // 7 bytes

const delta = text.sliceDeltaUtf8(start, end);
expect(delta).toStrictEqual([{ insert: "ðŸ‘‹" }]);
```
...
const slice2 = text.slice(6, 8); // "ðŸ˜€" (emoji spans 6-8)
...
**âš ï¸ Important:** Emoji and other characters outside the Basic Multilingual Plane count as 2 UTF-16 units. This affects all index-based operations:

```typescript no_run
text.insert(0, "ðŸ‘¨â€ðŸ‘©â€ðŸ‘§â€ðŸ‘¦"); // Family emoji
...
**âš ï¸ Important: List vs Map for Coordinates**

```typescript no_run
// âŒ WRONG - Don't use List for coordinates
const coord = doc.getList("coord");
coord.push(10); // x
coord.push(20); // y
// Concurrent updates can create [10, 20a, 20b] instead of [10, 20]

// âœ… CORRECT - Use Map for coordinates
...
**âš ï¸ Pitfall:** Concurrent regular child container creation at the same key on a LoroMap from multiple peers causes overwrites (different container IDs cannot auto-merge). If the child should be shared by logical key, use the `ensureMergeable*` methods instead. See [Container initialization](/docs/tutorial/tips).
...
**âš ï¸ Important Tree Operation Notes:**
...
âš ï¸ **Warning**: Manual assignment requires careful conflict avoidance.
...
- Allow multiple browser tabs or processes to operate on the same reused Peer ID in parallelâ€”coordinate access with locks so that only one session emits operations for that ID at a time
...
âš ï¸ These are independent concepts - a container can be attached to a detached document.
...
- Merging is monotonic: remote edits cannot be â€œrolled backâ€ on arrival
...
- âœ… Collaborative text, lists, maps, rich presence
- âœ… Comments, reactions, drafts, annotations
- âœ… Offline edits where convergence is enough
...
âš ï¸ **Important**: Peers can only sync if they have versions after the shallow snapshot point.
...
Simulate `âˆšN` concurrent actions. We measure the time to perform the task and
...
decoded document (`memUsed`). The logarithm of `N` was chosen because `âˆšN`
concurrent actions may result in up to `âˆšN^2 - 1` conflicts (apply action 1: 0
...
| [B3.1] 20âˆšN clients concurrently set number in Map (time)                |            54 ms |           60 ms |            27 ms |            32 ms |         1,058 ms |           21 ms |
| [B3.1] 20âˆšN clients concurrently set number in Map (updateSize)          |     49,167 bytes |    49,162 bytes |    132,376 bytes |     63,832 bytes |    283,296 bytes |   283,296 bytes |
| [B3.1] 20âˆšN clients concurrently set number in Map (encodeTime)          |             3 ms |            1 ms |            16 ms |             1 ms |             9 ms |           12 ms |
| [B3.1] 20âˆšN clients concurrently set number in Map (docSize)             |     36,763 bytes |    36,751 bytes |     78,764 bytes |     38,428 bytes |     86,165 bytes |    86,164 bytes |
| [B3.1] 20âˆšN clients concurrently set number in Map (parseTime)           |            57 ms |           67 ms |            83 ms |            49 ms |            59 ms |           54 ms |
| [B3.2] 20âˆšN clients concurrently set Object in Map (time)                |            55 ms |           62 ms |            27 ms |            40 ms |         1,126 ms |           28 ms |
| [B3.2] 20âˆšN clients concurrently set Object in Map (updateSize)          |     85,084 bytes |    85,073 bytes |    171,370 bytes |     99,753 bytes |    398,090 bytes |   325,370 bytes |
| [B3.2] 20âˆšN clients concurrently set Object in Map (encodeTime)          |             2 ms |            1 ms |            18 ms |             2 ms |            21 ms |           17 ms |
| [B3.2] 20âˆšN clients concurrently set Object in Map (docSize)             |     72,682 bytes |    72,659 bytes |     84,255 bytes |     75,227 bytes |    112,588 bytes |    93,401 bytes |
| [B3.2] 20âˆšN clients concurrently set Object in Map (parseTime)           |            73 ms |           68 ms |            83 ms |            48 ms |            68 ms |           59 ms |
| [B3.3] 20âˆšN clients concurrently set String in Map (time)                |           124 ms |           61 ms |            63 ms |            73 ms |         1,869 ms |          166 ms |
| [B3.3] 20âˆšN clients concurrently set String in Map (updateSize)          |  7,826,229 bytes | 7,826,225 bytes |  7,912,520 bytes |  7,840,917 bytes |  8,063,440 bytes | 8,063,440 bytes |
| [B3.3] 20âˆšN clients concurrently set String in Map (encodeTime)          |            56 ms |            5 ms |            70 ms |            23 ms |            59 ms |           61 ms |
| [B3.3] 20âˆšN clients concurrently set String in Map (docSize)             |  7,813,826 bytes | 7,813,814 bytes |    241,646 bytes |  7,815,537 bytes |     97,997 bytes |    98,008 bytes |
| [B3.3] 20âˆšN clients concurrently set String in Map (parseTime)           |           141 ms |           75 ms |           161 ms |            44 ms |            80 ms |           76 ms |
| [B3.4] 20âˆšN clients concurrently insert text in Array (time)             |            45 ms |           49 ms |           195 ms |            28 ms |         1,768 ms |           19 ms |
| [B3.4] 20âˆšN clients concurrently insert text in Array (updateSize)       |     52,751 bytes |    52,735 bytes |    137,490 bytes |     70,514 bytes |    311,830 bytes |   285,330 bytes |
| [B3.4] 20âˆšN clients concurrently insert text in Array (encodeTime)       |             1 ms |            0 ms |            13 ms |             1 ms |            13 ms |            7 ms |
| [B3.4] 20âˆšN clients concurrently insert text in Array (docSize)          |     26,596 bytes |    26,580 bytes |    100,791 bytes |     47,943 bytes |     96,423 bytes |    86,519 bytes |
| [B3.4] 20âˆšN clients concurrently insert text in Array (parseTime)        |            55 ms |           37 ms |            94 ms |            30 ms |            43 ms |           37 ms |
...
| [B3.5] 20âˆšN clients concurrently insert text (time)                      |            42 ms |           49 ms |           212 ms |           115 ms |         1,869 ms |           36 ms |
| [B3.5] 20âˆšN clients concurrently insert text (updateSize)                |     48,129 bytes |    48,135 bytes |    132,870 bytes |     68,978 bytes |    298,020 bytes |   298,020 bytes |
| [B3.5] 20âˆšN clients concurrently insert text (encodeTime)                |             1 ms |            0 ms |            13 ms |             1 ms |            12 ms |           16 ms |
| [B3.5] 20âˆšN clients concurrently insert text (docSize)                   |     24,313 bytes |    24,325 bytes |    102,818 bytes |     48,053 bytes |     90,768 bytes |    90,781 bytes |
| [B3.5] 20âˆšN clients concurrently insert text (parseTime)                 |            38 ms |           38 ms |            64 ms |            63 ms |            67 ms |           55 ms |
...
# FILE: pages/docs/advanced/jsonpath.mdx
...
> âš ï¸ **Illustration only**: This approach stuffs plain JavaScript objects into a
...
- **Selectors** â€“ names, indices (positive/negative), slices, unions, wildcards,
  and recursive descent (`..`).
- **Filters** â€“ logical operators (`&&`, `||`, `!`), comparisons, membership via
  `in`, substring checks with `contains`, property existence, and nested
  subqueries through `$` (root) or `@` (current node).
- **Functions** â€“ `count()`, `length()`, and `value()` are parsed, while `match()`
...
<details>
<summary>âš ï¸ DOMContentLoaded Timing Issue with Vite</summary>
...
| :-------------------------------------------------------------------------- | :--- | :------------ | :---------- | :---------- |
| [Event Graph Walker](https://loro.dev/docs/advanced/replayable_event_graph) | âœ…   | âœ… Inventor   | âŒ          | âŒ          |
| Rich Text CRDT                                                              | âœ…   | âŒ            | âŒ          | âœ…          |
| [Movable Tree](https://ieeexplore.ieee.org/document/9563274)                | âœ…   | âŒ            | âŒ          | âŒ Inventor |
| [Movable List](https://loro.dev/docs/tutorial/list)                         | âœ…   | âŒ            | âŒ          | âŒ Inventor |
| Time Travel                                                                 | âœ…   | âœ…            | âœ…[1]       | âœ…          |
| [Fugue](https://arxiv.org/abs/2305.00583) / Maximal non-interleaving        | âœ…   | âœ…            | âŒ          | âŒ          |
| JSON Types                                                                  | âœ…   | â“            | âœ…          | âœ…          |
| [Mergeable Containers](/blog/mergeable-containers)                          | âœ…   | âŒ            | âŒ          | âŒ          |
| Merging Elements in Memory by Run Length Encoding                           | âœ…   | âœ…            | âœ… Inventor | âŒ          |
| Byzantine-fault-tolerance                                                   | âŒ   | âŒ            | âŒ          | âœ…          |
| Version Control                                                             | âœ…   | âŒ            | âŒ          | âœ…          |
...
In the JavaScript package `loro-crdt@1.8.0`, we changed event emission from **â€œafter a microtaskâ€** to **synchronous** dispatch. This makes event handling simpler and less errorâ€‘prone.
...
However, this deferral made app logic fragile. There was a window between the mutation and the event callback. Example: the app receives the event from CRDT â€œdelete the 3rd character of `Hi!`â€, but during the microtask gap the app state changed `Hi!` â†’ `Hi`. When the deferred event arrives, applying a â€œdelete index 2â€ delta can be wrong. In practice, users had to maintain an awkward invariant: **donâ€™t mutate the app state during that microtask** if you consume delta updates.
...
- We keep a **global queue** of â€œpending (listener, event)â€ pairs.
...
- **Fewer footguns with deltas.** You no longer need to uphold â€œdonâ€™t touch the doc during the microtaskâ€ when applying deltaâ€‘style updates.
- **Listeners can still safely use `doc`.**


# FILE: pages/changelog/v1.9.0.mdx
...
<aside>
ðŸ’¡ **Differences between Loro and Eg-walker**
...
<aside>
ðŸ’¡ **Loro performs integrity checks before importing updates/snapshots**
...
# FILE: pages/blog/mergeable-containers.mdx
...
ðŸ¤:$state>note-1        type = Map

Nested key "body" under that mergeable map, child text:
ðŸ¤:$state>note-1>body   type = Text
```

Parsing the second CID gives:

```text
parent = Root("ðŸ¤:$state>note-1", Map)
key = "body"
type = Text
```
...
- User-created root names that start with the internal `ðŸ¤:` prefix are rejected by Loro's root-name validator, so they cannot collide with mergeable CIDs.
...
### Loro â€” the bug, and the fix

```js no_run
// loro.mjs  â€”  node loro.mjs
...
### Yjs â€” the same problem

```js no_run
// yjs.mjs  â€”  node yjs.mjs
...
### Automerge â€” the same problem

```js no_run
// automerge.mjs  â€”  node automerge.mjs
...
// -> ["A","B"] (order may vary): both survive â€” this is the eager-init workaround.
```

Note one difference worth calling out: in Automerge the losing child is retained and can be recovered through `getConflicts()`, while Yjs overwrites the map key and drops the losing child outright. Either way, from the application's point of view it looks like data loss â€” which is exactly what Mergeable Containers avoid.
...
> ðŸ’¡ The specific definition of user intent in the context of concurrent rich text editing can't be clearly explained in a few words. it's best understood through specific examples.
...
text.insert(0, "ä½ å¥½ï¼Œä¸–ç•Œï¼");
text.insert(2, "å‘€");
expect(text.toString()).toBe("ä½ å¥½å‘€ï¼Œä¸–ç•Œï¼");
...
expect(b.toString()).toBe("ä½ å¥½å‘€ï¼Œä¸–ç•Œï¼");
```
...
> 2. Render the cycle nodes (and their descendants) in a special â€œtime-outâ€ zone. They will stay there until some user manually fixes the cycle.
...
> 5. For forests: Within each cycle, letÂ `B.parent = A`Â be the edge whoseÂ `set`Â operation has the largest LWW timestamp. At render time, â€œhideâ€ that edge, instead renderingÂ `B.parent = "none"`, but donâ€™t change the actual CRDT state. This hides one of the concurrent edges that created the cycle.
>    â€¢ To prevent future surprises, usersâ€™ apps should follow the rule: before performing any operation that would create or destroy a cycle involving a hidden edge, first â€œaffirmâ€ that hidden edge, by performing an op that setsÂ `B.parent = "none"`.
...
# FILE: pages/blog/loro-protocol.mdx
...
For an update, if **k** items change and each changed item affects **m** of its immediate fields, time complexity is **â‰ˆ O(kÂ·m)**. _(k = number of changed items; m = average number of changed immediate fields per changed item.)_ This is similar to Reactâ€™s render complexity.
...
- Tree. LoroTree CRDT already ensures correct concurrent moves, but developers still translate tree operations into applicationâ€‘state patches. Mirror carries firstâ€‘class mappings from tree events into your state shape, so consumers can work with natural â€œinsert/move/delete nodeâ€ updates.
...
# FILE: .open-next/server-functions/default/pages/docs/api/js.mdx
