...
pub(super) fn paint_layout(
  ...
  for selection in external_selections {
    paint_selection_with_color(
      layout,
      &selection.selection,
      bounds.origin,
      content_mask,
      visible_range.clone(),
      remote_selection_color(selection.color_rgb),
      window,
    );
  }
  for paragraph in &layout.paragraphs[visible_range.clone()] {
    ...
    && caret.intersects(&content_mask)
  {
    ...
    let caret_color = local_caret_color_rgb.map_or_else(|| Background::from(black()), |color| Background::from(rgb(color)));
...
#[hotpath::measure]
pub(super) fn paint_selection_with_color(
  layout: &LayoutState,
  selection: &EditorSelection,
  origin: Point<Pixels>,
  content_mask: Bounds<Pixels>,
...
fn remote_selection_color(color_rgb: u32) -> Hsla {
  Hsla::from(rgb(color_rgb)).alpha(0.20)
}
