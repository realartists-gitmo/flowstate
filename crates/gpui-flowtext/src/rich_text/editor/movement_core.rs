...
impl RichTextEditor {
  ...
  pub(super) fn after_text_mutation(&mut self, cx: &mut Context<Self>) {
    self.after_text_mutation_with_scroll(true, cx);
  }

  pub(super) fn after_remote_text_mutation(&mut self, cx: &mut Context<Self>) {
    self.after_text_mutation_with_scroll(false, cx);
  }

  fn after_text_mutation_with_scroll(&mut self, scroll_to_head: bool, cx: &mut Context<Self>) {
