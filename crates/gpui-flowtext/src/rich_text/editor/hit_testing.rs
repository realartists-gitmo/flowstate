...
impl RichTextEditor {
  ...
  fn backspace(&mut self, cx: &mut Context<Self>) {
    ...
    if caret.byte == 0 {
      ...
      if !delete_cross_paragraph_range(
        &mut self.document,
        DocumentOffset {
          paragraph: prev_ix,
          byte: prev_len,
      ...
      if !delete_range_in_paragraph(&mut self.document, caret.paragraph, prev..caret.byte) {
        return;
      }
      let new = DocumentOffset {
        paragraph: caret.paragraph,
  ...
  fn delete_forward(&mut self, cx: &mut Context<Self>) {
    ...
    if caret.byte == para_len {
      ...
      if !delete_range_in_paragraph(&mut self.document, caret.paragraph, caret.byte..next) {
        return;
      }
    }
    self.after_text_mutation(cx);
  ...
  fn insert_paragraph_break(&mut self, cx: &mut Context<Self>) {
    if !self.selection.is_caret() && !self.delete_selection_internal() {
      return;
    }
    let caret = self.selection.head;
    ...
    let starts_empty_paragraph = caret.byte == paragraph_text_len(paragraph);
    if !split_paragraph_at(&mut self.document, caret.paragraph, caret.byte) {
      return;
    }
    let new = DocumentOffset {
      paragraph: caret.paragraph + 1,
