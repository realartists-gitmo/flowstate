...
impl Db8CollabAdapter {
  ...
  fn adapt_run_diffs(result: &mut Db8CollabAdapterResult, text_id: String, before: &super::Paragraph, after: &super::Paragraph) {
    ...
    for (before_run, after_run) in before.runs.iter().zip(&after.runs) {
      ...
      if before_run.len == after_run.len {
        offset += before_run.len;
      } else {
        offset += before_run.len.min(after_run.len);
        repaired = true;
      }
    }
    if repaired || before.runs.len() != after.runs.len() {
      result.repair_required = true;
    }
  }
}
...
impl CollaborationEdit {
  ...
  pub fn from_operations(operations: Vec<CanonicalOperation>) -> Self {
    ...
    Self {
      ...
      repair_required: adapter.repair_required,
    }
  }
}
...
mod tests {
  ...
  #[test]
  fn unsupported_structural_operations_are_marked_for_repair() {
  ...
  #[test]
  fn structural_operations_can_still_be_sent_as_application_hints_when_source_needs_repair() {
    ...
    assert!(edit.source_mutations.is_empty());
