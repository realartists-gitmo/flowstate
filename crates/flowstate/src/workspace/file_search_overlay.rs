...
impl FileSearchOverlay {
  pub fn new(
    ...
    let search_input = cx.new(|cx| InputState::new(window, cx).placeholder("file_name.db8, file_name.docx, or file_name.pdf"));
              ...
              .when(!has_hits, |this| {
                let message = self.error.clone().unwrap_or_else(|| {
                  if self.loading {
                    ...
                    "No .db8, .docx, or .pdf files indexed"
                  } else {
                    "No matching .db8, .docx, or .pdf files"
                  }
                  .into()
                });
                this.child(
