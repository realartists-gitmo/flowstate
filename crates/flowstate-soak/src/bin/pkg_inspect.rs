...
fn main() -> anyhow::Result<()> {
  ...
  println!(
    "phase document_from_loro: {:?} ({} paragraphs)",
    started.elapsed(),
    projection.paragraphs.len()
  );
  let snapshot_bytes: usize = package.loro_snapshots.iter().map(|c| c.bytes.len()).sum();
  let segment_bytes: usize = package
    .loro_update_segments
    .iter()
    .map(|c| c.bytes.len())
    .sum();
  let asset_bytes: usize = package
    .assets
    .iter()
    .map(|c| c.bytes.len() + c.metadata.len())
    .sum();
  let cache_bytes: usize = package
    .projection_caches
    .iter()
    .map(|c| c.bytes.len())
    .sum();
  let search_bytes: usize = package
    .search_units
    .iter()
    .map(|c| c.frontier.len() + c.unit_kind.len() + 64)
    .sum();
  ...
  encoded(
    "manifest",
    postcard::to_stdvec(&package.manifest)
      .map(|b| b.len())
      .unwrap_or(0),
  );
  encoded(
    "snapshots",
    postcard::to_stdvec(&package.loro_snapshots)
      .map(|b| b.len())
      .unwrap_or(0),
  );
  encoded(
    "segments",
    postcard::to_stdvec(&package.loro_update_segments)
      .map(|b| b.len())
      .unwrap_or(0),
  );
  encoded(
    "assets",
    postcard::to_stdvec(&package.assets)
      .map(|b| b.len())
      .unwrap_or(0),
  );
  encoded(
    "revisions",
    postcard::to_stdvec(&package.revisions)
      .map(|b| b.len())
      .unwrap_or(0),
  );
  encoded(
    "projection_caches",
    postcard::to_stdvec(&package.projection_caches)
      .map(|b| b.len())
      .unwrap_or(0),
  );
  encoded(
    "search_units",
    postcard::to_stdvec(&package.search_units)
      .map(|b| b.len())
      .unwrap_or(0),
  );
  encoded(
    "thumbnails",
    postcard::to_stdvec(&package.thumbnails)
      .map(|b| b.len())
      .unwrap_or(0),
  );
  encoded(
    "integrity_index",
    postcard::to_stdvec(&package.integrity_index)
      .map(|b| b.len())
      .unwrap_or(0),
  );
