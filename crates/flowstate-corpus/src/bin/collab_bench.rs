...
fn main() {
  ...
  eprintln!("OPEN            {:8.1} ms   ({paragraph_count} paragraphs)", open.elapsed().as_secs_f64() * 1000.0);
  ...
  for _ in 0..rounds {
    ...
    doc_b.commit();
    let update = doc_b.export(loro::ExportMode::updates(&peer_vv)).expect("peer export");
  ...
  eprintln!("IMPORT          {:8.1} ms   ({rounds} rounds, {:.2} ms/round)", import_ms, import_ms / rounds as f64);
  ...
  for _ in 0..rounds {
    let t = Instant::now();
    handle
      .insert_text(InsertTextIntent { at: TextAnchor::new(first, 0), text: "y".to_string(), style_override: None })
      .expect("insert_text");
    key_ms += t.elapsed().as_secs_f64() * 1000.0;
  }
  eprintln!("KEYSTROKE       {:8.1} ms   ({rounds} rounds, {:.2} ms/keystroke)", key_ms, key_ms / rounds as f64);
  ...
  handle
    .set_paragraph_styles(SetParagraphStylesIntent { paragraphs: paragraph_ids.to_vec(), style: ParagraphStyle::Custom(2) })
    .expect("set_paragraph_styles");
  eprintln!("MASS-RESTYLE    {:8.1} ms   ({} paragraphs)", mass.elapsed().as_secs_f64() * 1000.0, paragraph_ids.len());
  ...
  for round in 0..rounds {
    ...
    let t = Instant::now();
    handle
      .split_paragraph(SplitParagraphIntent { at: TextAnchor::new(mid, 1), inherited_style: ParagraphStyle::Normal })
    ...
    let t = Instant::now();
    handle
      .join_paragraphs(JoinParagraphsIntent { first: first_id, second: second_id })
  ...
  let mut peer_handle_vv = {
    let guard = peer_gate.lock(GateHolder::ExportUpdates).expect("peer gate");
    guard.doc().state_vv()
  };
  ...
  for round in 0..rounds {
    ...
    let update = {
      let guard = peer_gate.lock(GateHolder::ExportUpdates).expect("peer gate");
      let update = guard.doc().export(loro::ExportMode::updates(&peer_handle_vv)).expect("peer export");
  ...
  let mut import_peer_chunk = |label: &str| {
    ...
    let events = guard.import_remote_update(&update).expect("import peer mass chunk");
  ...
  let peer_paragraphs = peer_handle.projection().expect("peer projection").ids.paragraph_ids.clone();
  peer_handle
    .set_paragraph_styles(SetParagraphStylesIntent { paragraphs: peer_paragraphs.to_vec(), style: ParagraphStyle::Custom(3) })
  ...
  // (a same-shape proxy for find&replace-all); A imports one chunk.
  {
    ...
    for block in peer_projection.blocks.iter() {
      if let flowstate_document::Block::Paragraph(paragraph) = block {
        ...
        if paragraph.runs.iter().map(|run| run.len).sum::<usize>() >= 3 {
          matches.push(ReplaceMatch { start: TextAnchor::new(id, 0), end: TextAnchor::new(id, 1), styles: None });
        }
        paragraph_ix += 1;
      }
    }
    ...
    peer_handle
      .replace_matches(ReplaceMatchesIntent { matches, replacement: "Q".to_string() })
  ...
  // imports one chunk.
  {
    ...
    peer_handle.delete_range(DeleteRangeIntent { start, end }).expect("peer mass delete")
  };
  ...
  // UNDO, which stays fast-path).
  {
    let live = handle.projection().expect("projection");
    handle
      .set_paragraph_styles(SetParagraphStylesIntent { paragraphs: live.ids.paragraph_ids.to_vec(), style: ParagraphStyle::Custom(4) })
      .expect("mass restyle before remote chatter");
    let peer_first = peer_handle.projection().expect("peer projection").ids.paragraph_ids[0];
    peer_handle
      .insert_text(InsertTextIntent { at: TextAnchor::new(peer_first, 0), text: "r".to_string(), style_override: None })
    ...
    eprintln!("UNDO-AFTER-REMOTE {:8.1} ms   (select-all restyle: v2 per-coordinate rebase keeps the fast path)", t.elapsed().as_secs_f64() * 1000.0);
  ...
  {
    let live = handle.projection().expect("projection");
    let bottom_half: Vec<_> = live.ids.paragraph_ids.iter().skip(live.ids.paragraph_ids.len() / 2).copied().collect();
    handle
      .set_paragraph_styles(SetParagraphStylesIntent { paragraphs: bottom_half, style: ParagraphStyle::Custom(5) })
      .expect("partial mass restyle");
    let peer_first = peer_handle.projection().expect("peer projection").ids.paragraph_ids[1];
    peer_handle
      .insert_text(InsertTextIntent { at: TextAnchor::new(peer_first, 0), text: "t".to_string(), style_override: None })
    ...
    eprintln!("UNDO-AFTER-REMOTE-PARTIAL {:8.1} ms   (rebase keeps the fast path)", t.elapsed().as_secs_f64() * 1000.0);
  ...
  // dirty history). Gate: same order as a no-chatter undo, zero fallbacks.
  {
    ...
    handle
      .insert_text(InsertTextIntent { at: TextAnchor::new(deep, 0), text: "k".to_string(), style_override: None })
  ...
  eprintln!("REMOTE-STRUCT-BARE {:8.1} ms   ({rounds} rounds, {:.2} ms/round)", bare_ms, bare_ms / rounds as f64);
