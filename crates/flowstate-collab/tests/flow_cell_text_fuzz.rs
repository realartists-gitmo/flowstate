      ...
      rows
        ...
        .filter_map(|block| match block {
          gpui_flowtext::InputBlock::Paragraph(paragraph) => {
            Some(paragraph.runs.iter().map(|run| run.text.as_str()).collect::<String>())
          },
          _ => None,
        })
        .collect::<Vec<_>>()
  ...
  for _ in 0..2 {
    for a in 0..peers.len() {
      for b in 0..peers.len() {
        ...
        if let Ok(bytes) = peers[a].handle.with_test_runtime(|runtime| runtime.export_updates_for(&vv))
          && !bytes.is_empty()
        {
          peers[b]
            .handle
  ...
  for round in 0..rounds {
    for op in 0..ops_per_round {
      ...
      if rng.below(8) == 0 {
        ...
        let cells: Vec<CellId> = board.sheet(sheet).expect("sheet").cells.iter().map(|cell| cell.id).collect();
        if cells.len() > 1 {
...
fn reorder_never_loses_a_concurrent_keystroke() {
  ...
  let projection = peers[0].authority.canonical_projection().expect("projection");
  ...
  for peer in &peers {
    ...
    let order: Vec<CellId> = board.sheet(sheet).expect("sheet").cells.iter().map(|cell| cell.id).collect();
