...
impl Fixture {
  ...
  fn cell_ids(&self) -> Vec<CellId> {
    self.board().sheets[0].cells.iter().map(|cell| cell.id).collect()
  }
...
fn structural_intents_maintain_the_board_in_place() {
  ...
  let board = fx.board();
  assert_eq!(
    board.sheets.iter().map(|sheet| sheet.id).collect::<Vec<_>>(),
  ...
  fx.handle
    ...
    .expect("delete cell");
  let board = fx.board();
  assert_eq!(board.sheet(fx.sheet).unwrap().cell(response).unwrap().parent_id, None);
  assert_eq!(board.sheet(fx.sheet).unwrap().cell(first_response).unwrap().parent_id, None);
...
fn cell_text_intents_drive_the_cell_stream_and_summary() {
  ...
  authority
    ...
    .expect("marks");
  let projection = authority.canonical_projection().expect("projection");
  assert!(projection.paragraphs[0].runs.iter().any(|run| run.styles.strikethrough));
  assert_eq!(projection.paragraphs[1].style, flowstate_document::PARAGRAPH_ANALYTIC);

  // Join back + delete a range.
  authority
    .apply(LocalIntent::JoinParagraphs(JoinParagraphsIntent {
      first: paragraph,
      second,
    }))
    .expect("join");
...
fn strike_and_editable_and_annotations() {
  ...
  assert!(!fx.board().cell(cell).unwrap().1.summary.uses_summary_projection);
...
fn undo_meta_restores_board_focus_context() {
  let fx = fixture();
  let cell = fx.add_cell(CellPlacement::ColumnEnd { column_index: 0 });
  fx.handle.set_undo_context(flowstate_collab::flow::runtime::FlowUndoMeta {
...
fn publish_queue_converges_a_second_runtime() {
  ...
  let events = fx.handle.with_test_runtime(FlowRuntime::take_pending_publish);
  let mut imported = false;
  for event in events {
    if let FlowPublishEvent::LocalUpdate { bytes, .. } = event {
      peer_b.with_test_runtime(|runtime| runtime.import_remote_updates(&[bytes.as_slice()]).expect("import"));
      imported = true;
    }
  }
