...
mod tests {
  ...
  fn seeded() -> (LocalDocHandle, std::sync::Arc<flowstate_collab::local_write::WriteGate<CrdtRuntime>>, LoroDoc) {
    ...
    for i in 0..200 {
      handle
        .insert_text(InsertTextIntent { at: TextAnchor::new(para, usize::MAX), text: format!("word{i} "), style_override: None })
        .expect("seed");
    }
    let peer = {
      let g = gate.lock(GateHolder::ExportUpdates).expect("gate");
      let doc = LoroDoc::new();
      doc.import(&g.doc().export(loro::ExportMode::updates(&loro::VersionVector::default())).expect("export")).expect("peer boot");
      doc
    };
    (handle, gate, peer)
  }
  ...
  fn push_peer(gate: &std::sync::Arc<flowstate_collab::local_write::WriteGate<CrdtRuntime>>, peer: &LoroDoc, from: &loro::VersionVector) {
    let update = peer.export(loro::ExportMode::updates(from)).expect("peer export");
    gate.lock(GateHolder::ImportChunk).expect("gate").import_remote_update(&update).expect("import");
  }
  ...
  fn mark_overlap_undo_timing() {
    ...
    handle
      .set_paragraph_styles(SetParagraphStylesIntent { paragraphs: ids.to_vec(), style: ParagraphStyle::Custom(3) })
      .expect("A restyle");
