...
fn resolve_intent(core: &CrdtRuntime, intent: &LocalIntent) -> Result<ResolvedPlan, WriteRejected> {
  ...
  match intent {
    ...
    // Cell-text edits are resolved and committed by the FLOW runtime's
    // `flow/cell_text.rs` under its own write gate — the .db8 document commit
    // path never sees them.
    LocalIntent::TableCellText(_) => Err(WriteRejected::StructureViolation(
      "TableCellText is a flow-cell op; it cannot resolve against the document body",
    )),
  }
}
